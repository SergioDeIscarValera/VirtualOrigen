...
class InversorChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    var count = -1;
    final batteryDisplay =
        batteryData.sublist(offset - displayCount, offset).map((e) {
      count++;
      return FlSpot(count.toDouble(), e.y);
    }).toList();
    count = -1;
    final gainDisplay =
        gainData.sublist(offset - displayCount, offset).map((e) {
      count++;
      return FlSpot(count.toDouble(), e.y);
    }).toList();
    count = -1;
    final consumptionDisplay =
        consumptionData.sublist(offset - displayCount, offset).map((e) {
      count++;
      return FlSpot(count.toDouble(), e.y);
    }).toList();
